...
bird_eye_image = np.zeros((480, 640, 1), dtype=np.uint8)
...
# 정지선인지?
def is_stopline(bird_eye_frame):
    """
    Bird-Eye View 프레임에서 ROI 영역 내 수직선 개수를 바탕으로
    횡단보도인지 여부를 판단
    """
    ...
    if lines is not None:
        ...
        for line in lines:
            ...
            if x2 - x1 == 0:
                ...
                slope = float(y2 - y1) / (x2 - x1)
            if abs(slope) <= 1.0:  # 수직선 기준
                diagonal_count += 1
            
        print(diagonal_count)
        
    # 기준 이상이면 횡단보도로 판단
    return diagonal_count >= 1  # 수직선 4개 이상이면 횡단보도
...
def change_brid_eye(raw_image):
    global calibration_image, bird_eye_image, bird_eye_roi
...
def start():
    ...
    stopline_frame_count = 0                # 정지선 체크 카운터
    # 여기서 img_callback
    ...
    while not rospy.is_shutdown():
        ...
        if is_stopline(bird_eye_image) and not stopline_stop_completed:
            stopline_frame_count += 1
            if stopline_frame_count > 5:
                drive(0, 0)
                print"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
                time.sleep(5)
                stopline_stop_completed = True
        else:   
            stopline_frame_count = 0
